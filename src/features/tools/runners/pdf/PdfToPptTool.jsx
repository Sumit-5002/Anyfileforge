import React, { useState } from 'react';
import pdfService from '../../../../services/pdfService';
import FileUploader from '../../../../components/ui/FileUploader';
import ToolWorkspace from '../common/ToolWorkspace';
import { Presentation, CircleCheck, FileText } from 'lucide-react';

function PdfToPptTool({ tool, onFilesAdded }) {
    const [file, setFile] = useState(null);
    const [processing, setProcessing] = useState(false);
    const [done, setDone] = useState(false);

    const handleFilesSelected = (files) => {
        setFile(files[0]);
        setDone(false);
        if (onFilesAdded) onFilesAdded(files);
    };

    const handleProcess = async () => {
        if (!file) return;
        setProcessing(true);
        try {
            await pdfService.pdfToPPT(file);
            setDone(true);
        } catch (error) {
            console.error('PDF to PPT error:', error);
            alert('Failed to convert PDF to PowerPoint.');
        } finally {
            setProcessing(false);
        }
    };

    if (!file) {
        return <FileUploader tool={tool} onFilesSelected={handleFilesSelected} multiple={false} />;
    }

    return (
        <ToolWorkspace
            tool={tool}
            files={[file]}
            onFilesSelected={handleFilesSelected}
            onReset={() => { setFile(null); setDone(false); }}
            processing={processing}
            onProcess={handleProcess}
            actionLabel="Convert to PowerPoint"
            sidebarTitle="Export Settings"
            sidebar={
                <div className="sidebar-info">
                    <p className="tool-help">
                        Converts your PDF pages into slides for a PowerPoint (.pptx) presentation.
                    </p>
                    <div className="mt-3 d-flex align-items-center gap-2">
                        <Presentation size={20} className="text-primary" />
                        <span className="small">Ready to convert {file.name}</span>
                    </div>
                </div>
            }
        >
            <div className="files-list-view" style={{ minHeight: '300px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {done ? (
                    <div className="fade-in text-center">
                        <CircleCheck size={64} className="text-success mb-3" />
                        <h3>Conversion Complete!</h3>
                        <p className="text-muted">Your PowerPoint presentation is ready.</p>
                    </div>
                ) : (
                    <div className="file-item-horizontal w-full" style={{ maxWidth: '500px' }}>
                        <FileText size={24} className="text-primary" />
                        <div className="file-item-info">
                            <div className="file-item-name">{file.name}</div>
                            <div className="file-item-size">{(file.size / 1024).toFixed(1)} KB</div>
                        </div>
                    </div>
                )}
            </div>
        </ToolWorkspace>
    );
}

export default PdfToPptTool;
